...
def launch_overlay():
    ...
    # Get the directory where plaix.py lives
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Define possible names for the electron executable on Windows
    electron_names = ["electron.cmd", "electron.exe", "electron"]
    
    # Define the bin folder
    bin_path = os.path.join(base_dir, "electron", "node_modules", ".bin")
    
    electron_bin = None
    
    # Search for any valid version of the electron command
    if os.path.exists(bin_path):
        for name in electron_names:
            test_path = os.path.join(bin_path, name)
            if os.path.exists(test_path):
                electron_bin = test_path
                break

    electron_main = os.path.join(base_dir, "electron", "main.js")

    if not electron_bin:
        # Fallback: Is it installed globally?
        electron_bin = "electron.cmd" 
        print("[WARN] Local Electron not found, trying global 'electron.cmd'")

    try:
        # We use shell=True because .cmd files are technically shell scripts
        subprocess.Popen([electron_bin, electron_main], shell=True)
        print(f"[OVERLAY] Started: {electron_bin}")
    except Exception as e:
        print(f"[OVERLAY] Launch failed: {e}")

# ==========================================
# TRIGGER THE LAUNCH
# ==========================================
import threading
def delayed_overlay_launch():
    time.sleep(3)  # Give the main app a head start
    launch_overlay()

# Start the launcher thread
threading.Thread(target=delayed_overlay_launch, daemon=True).start()
